...
const BookingModal = ({ service, visible, roomId, onClose, }) => {
    ...
    //         if (addon.is_required === "1") {
    //             defaultAddons[addon.id] = {
    //                 ...addon,
    //                 selected: true,
    //                 qty: addon.price_type === "unit" ? parseInt(addon.qty) : calculateSqftQty(addon)
    //             };
    //         }
    //     });
    ...
    useEffect(() => {
        ...
        setTotalPrice(calculatedQuantity * (service?.rate || 0));
    }, [widthValue, heightValue, otherValue, service]);
    ...
    const addToCart = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                rate: service?.rate,
                amount: totalPrice,
                reference_image: JSON.stringify(uploadedImageUrls), // Use uploaded image URLs
                addons: addonsArray
            };

            const response = await apiRequest("POST", "/seeb-cart/save", payload);
                    ...
                    <ScrollView showsVerticalScrollIndicator={false} style={{}}>
                        ...
                        {loading ? <TouchableOpacity style={styles.addButton}>
                            ...
                            <TouchableOpacity style={styles.addButton} onPress={() => addToCart()}>
                                ...
                                <Text style={styles.addButtonText}>Add ₹ {(totalPrice + totalAddonAmount).toFixed(2)}</Text>
